...
pub fn build_entity(ecs: &mut Ecs, new_entity_data: NewEntity) {
    ...
    unsafe {entity_data.assume_safe() }.map(|data: &EntityResource, _ |{
        ...
        let entity_root_node: Ref<Node2D> = unsafe { entity_root_node.assume_safe() }.cast::<Node2D>().unwrap().claim();
        unsafe { entity_root_node.assume_safe() }.set("position", new_entity_data.position.unwrap());
        unsafe { entity_root_node.assume_safe() }.set("entity_id", (entity.to_bits().get() as i64).to_variant());
        builder.add(GodotNode { godot_entity: entity_root_node });
        if data.components.is_some() {
            ...
            for some_variant in components.iter() {
                ...
                match &component_resource[..] {
                    // TODO - these could be generated by build.rs
        ...
        if data.is_ai_agent {
            ...
            if data.actions.is_some() {
                ...
                for sensor_variant in data.ai_components.as_ref().unwrap().iter() {
                    ...
                    match &component_resource[..] {
                        "FindObjectSensor" => { action_builder.add(FindObjectSensor::from_resource(sensor_resource));}
                        "Hunger" => { action_builder.add(Hunger::from_resource(sensor_resource));}
                        _ => {
